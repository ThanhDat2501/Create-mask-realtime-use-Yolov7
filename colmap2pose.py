...
def parse_args():
    parser = argparse.ArgumentParser(
        description="convert colmap to NeRF or LLFF(NeRD) format, automask objects, and process video")
    parser.add_argument('--json', action="store_true",
                        default='false', help='output json/instant-ngp format')
    parser.add_argument("--mask", action="store_true",
                        default="",
                        help="use rembg to automatically mask the entire image folder. reccomended to use manual masks for better results in nvdiffrec.")
    parser.add_argument("--alpha", action="store_true",
                        default="",
                        help="only works when --mask is enabled, applys a mask to the alpha channel of the image (instead of a separate folder). enables masking in instant-ngp.")
    parser.add_argument("--video_in",
                        default="",
                        help="run ffmpeg first to convert a provided video file into a set of images. uses the video_fps parameter also. needs ffmpeg in path or installed.")
    ...
    parser.add_argument('--match_type', type=str,
                        ...
                        help='type of matcher used.  Valid options: exhaustive_matcher sequential_matcher.  Other matchers not supported at this time')
    parser.add_argument("--database_db",
                        default="database.db", help="colmap database filename")
    parser.add_argument("--aabb_scale",
                        default=2, choices=["1", "2", "4", "8", "16"],
...
def run_colmap(basedir, match_type, colmap_path="Colmap"):
    ...
    feature_extractor_args = [
        ...
        '--ImageReader.mask_path', 'bb/masks',
...
def run_ffmpeg(args):
    ...
    do_system(
        f"ffmpeg -i {video} -qscale:v 1 -qmin 1 -vf \"fps={fps}{time_slice_value}, scale=-2:512\"  {os.path.join(args.scenedir, args.images)}\%04d.jpg")
...
if __name__ == '__main__':
    if args.video_in != "":
        run_ffmpeg(args)
    if args.mask != "":
        from rembg import removeq

        if args.alpha != "":
            print("applying alpha masks, images will be saved as _alpha.png (this is VERY slow)")
            try:
                os.mkdir(os.path.normpath(f"{args.scenedir}/images_alpha/"))
            except:
                pass
            ...
            if (input(f"please filter images with bad alpha in images_alpha. continue? (Y/n)").lower().strip() + "n")[
               :1] != "n":
                sys.exit(1)
            print(
                'images_alpha is the new default image folder, please do not rename it or the files inside after colmapping')
            args.images = "images_alpha"
        else:
            print("producing mask images")
            try:
                os.mkdir(os.path.normpath(f"{args.scenedir}/masks/"))
            except:
                pass
            for img in os.listdir(os.path.normpath(f"{args.scenedir}/{args.images}/")):
                im = cv2.imread(os.path.normpath(f"{args.scenedilr}/{args.images}/{img}"))
    ...
    if args.colmap_path != '':
        if os.name == 'nt' and "COLMAP.bat" not in args.colmap_path:
            ...
            sys.exit("colmap path fail, make sure your path is accessable")
    elif os.name == 'nt':
